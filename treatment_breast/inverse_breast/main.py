import os
from treatment_breast.inverse_breast.data_utils import prepare_data
from treatment_breast.inverse_breast.visualisation import visualize_solution_evolution
from treatment_breast.inverse_breast.model import DNN_builder
import tensorflow as tf
import matplotlib.pyplot as plt
import time
import numpy as np 
from treatment_breast.inverse_breast.train_batch_inverse import train_pinn

def run_experiment(config):

    # --- Create unique folder for experiment ---
    exp_id = f"{config['experiment_name']}"
    save_dir = os.path.join("results/experiments_biostec_25_06", exp_id)
