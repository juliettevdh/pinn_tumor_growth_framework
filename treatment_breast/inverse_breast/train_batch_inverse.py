### This script implements the training of the PINN model recovering
### treatment efficacy and using some batching to improve computation time. 
...
from treatment_breast.inverse_breast.data_utils import load_mat 
import cv2
from treatment_breast.inverse_breast.visualisation import plot_center_time_evolution
...
def train_pinn(model, data, config, save_dir, debug=False):
    ...
    alpha_dox = tf.Variable(tf.math.log(tf.constant(1e-1, dtype=tf.float64)), dtype=tf.float64, trainable= True)  # default value
    alphas = [alpha_dox]
    ...
    def compute_lambda_tf(v, t_norm):
        ...
        for i, drug in enumerate(drugs):
            ...
            for tau in times:
                ...
                decay = tf.exp(alpha_i) * tf.exp(-beta_i * (phys_t - tauf))
    ...
    def plot_lambda_map(model, config, t_phys, save_path=None):
        ...
        """
        t_max = 130
        t_norm = t_phys / t_max  # normalisé dans [0,1]
    ...
    for t in [0, 20, 21, 41, 42, 43, 63, 84, 104, 130]:
    ...
    # --- Training step ---
    @tf.function
    def train_step(x_d_b, y_d_b, t_d_b, u_d_b, x_c_b, y_c_b, t_c_b):
        with tf.GradientTape() as tape:
            # data losses
            u_pred_data = model(tf.concat([x_d_b, y_d_b, t_d_b], axis=1))
            l_data = mse(u_d_b, u_pred_data)

            # physics loss
            F = pde_residual(x_c_b, y_c_b, t_c_b)
            L_phys = tf.reduce_mean(tf.square(F))

            loss = alpha_weight * l_data + gamma * L_phys

        train_vars = model.trainable_weights
        grads = tape.gradient(loss, train_vars + [alpha_dox])
        opt.apply_gradients(zip(grads, train_vars + [alpha_dox]))

        return loss, l_data, L_phys, grads, alpha_dox
    ...
    dataset_data = tf.data.Dataset.from_tensor_slices((data['x_data'], data['y_data'], data['t_data'], data['u_data'])) \
                .shuffle(buffer_size=int(data['x_data'].shape[0])) \
    ...
    for epoch in range(epochs):
        x_d_b, y_d_b, t_d_b, u_d_b = next(iter_data)
        x_c_b, y_c_b, t_c_b = next(iter_c)

        loss, l_data, L_phys, grads, alpha_train_dox = train_step(
            tf.cast(x_d_b, tf.float64), tf.cast(y_d_b, tf.float64), tf.cast(t_d_b, tf.float64), tf.cast(u_d_b, tf.float64),
            tf.cast(x_c_b, tf.float64), tf.cast(y_c_b, tf.float64), tf.cast(t_c_b, tf.float64)
        )
        alpha_dox_list.append(tf.exp(alpha_train_dox.numpy()))

        if epoch % 1000 == 0:
            print(f"[Training] Epoch {epoch}/{epochs} Loss: {loss.numpy():.4e} (data: {l_data.numpy():.4e}, pde: {L_phys.numpy():.4e})")
            print(f"alpha_dox: {tf.exp(alpha_train_dox).numpy():.4e}")
            loss_history.append(loss.numpy())
            loss_data.append((l_data.numpy()))
            loss_pde.append(L_phys.numpy())
            grad_norms.append(tf.linalg.global_norm(grads).numpy())

        # visualize training points occasionally (less often)
        if epoch % 15000 == 0:
            plt.figure(figsize=(6,6))
            plt.scatter(data['x_data'].numpy().flatten(), data['y_data'].numpy().flatten(), c=data['u_data'].numpy().flatten(), s=1, label='Data')
    ...
    np.savetxt(os.path.join(save_dir, "alpha_history_dox.txt"), np.array(alpha_dox_list))
